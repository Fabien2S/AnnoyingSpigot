...
public abstract class GamePlayer implements ITickable, IStateMachine<GamePlayer>, IModifierCollection, IUnsafeEntityLocation, ISoundEmitter {

    private static final Logger LOGGER = LogManager.getLogger(GamePlayer.class);
    ...
    @Getter protected final IPlayerController controller;
    ...
    @Getter protected final MagicalCollection<Boolean> invisible = new MagicalCollection<>(false, Boolean::compareTo);

    private final Map<NamespacedKey, MagicalDouble> modifierMap = new HashMap<>();

    private Location playerLocation;
    private Location headLocation;
    @Nullable protected IState<GamePlayer> playerState;
    ...
    protected GamePlayer(AnnoyingPlugin plugin, NamespacedKey roleName, Player spigotPlayer, IPlayerControllerProvider controllerProvider) {
        this.plugin = plugin;
        this.roleName = roleName;
        this.spigotPlayer = spigotPlayer;
        this.controller = controllerProvider.provide(this);

        this.interactionManager = new InteractionManager(this);

        EntityRendererManager entityRendererManager = plugin.getEntityRendererManager();
        this.entityRendererManager = new EntityRendererManager(entityRendererManager, entity -> controller);
    ...
    public void emitSound(@NotNull Sound sound, @NotNull SoundCategory category, float volume, float pitch) {
        ...
        playerList.forEach(p -> {
            ...
            if (spigotPlayer.canSee(this.spigotPlayer)) {
                IPlayerController controller = p.getController();
                controller.playSound(this.spigotPlayer, sound, category, volume, pitch);
            }
        });
    }
    ...
    @Override
    public void setState(@Nullable IState<GamePlayer> playerState) {
        LOGGER.info("{}: {} -> {}", this, this.playerState, playerState);

        IState<GamePlayer> previousState = this.playerState;
